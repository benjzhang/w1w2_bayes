#!/bin/bash
# Test KL variational formulations: LT (current), LT_nu (trainable nu), DV (Donsker-Varadhan)
# The key hypothesis: LT without nu optimization suppresses gradients when distributions are close.
# LT_nu and DV should fix this by recentering the exponential.
...
# Use hard spectral norm (gp_weight=0) with moderate settings
DISC_STEPS=3
DISC_LR=0.001
GP_WEIGHT=0.0
N_TRAIN=10000
...
for PROBLEM in circle bimodal_quadratic; do
    ...
    for FORMULATION in LT LT_nu DV; do
        # === Flow-start ===
        JOB_NAME="gpa_form_${FORMULATION}_${PROBLEM}_fresh"
...
EOF
        ...
        # === Prior-start ===
        JOB_NAME="gpa_form_${FORMULATION}_${PROBLEM}_prior"
        echo "Submitting: $JOB_NAME"
        sbatch <<EOF
#!/bin/bash
#SBATCH -J ${JOB_NAME}
...
echo "All formulation sweep jobs submitted!"
